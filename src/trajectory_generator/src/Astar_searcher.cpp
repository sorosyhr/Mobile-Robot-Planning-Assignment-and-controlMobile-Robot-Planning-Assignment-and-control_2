...
/*double Astarpath::getHeu(MappingNodePtr node1, MappingNodePtr node2) {
  ...
  const double D = 1.0;
  const double D2 = sqrt(2.0);
  const double D3 = sqrt(3.0);
  
  // h
  double h = D * (dx + dy + dz) +
             (D2 - D) * std::min({dx, dy, dz}) +
             (D3 - D2) * (dx > 0 && dy > 0 && dz > 0 ? 1 : 0);

  //  tie breaker
  double tie_breaker = 1.0 + 1.0 / (GRID_X_SIZE + GRID_Y_SIZE + GRID_Z_SIZE);
  h *= tie_breaker;

  // p
  const double min_step_cost = D; // Minimum cost of one ste
  const double max_path_cost = (D * (GRID_X_SIZE + GRID_Y_SIZE + GRID_Z_SIZE)); // Expected maximum path cost
  double p = min_step_cost / max_path_cost;
  h *= (1.0 + p);
  return h;
}*/
...
bool Astarpath::AstarSearch(Vector3d start_pt, Vector3d end_pt) {
  ...
  while (!Openset.empty()) {
    ...
    for (unsigned int i = 0; i < neighborPtrSets.size(); i++) {
      ...
      if (neighborPtr->id == -1 || isOccupied(neighborPtr->index)) {
      ...
      if (neighborPtr->id == 0) {
        // If the node is not in the OpenSet, add it
      ...
      } else if (neighborPtr->id == 1 && tentative_g_score < neighborPtr->g_score) {
        // If the node is in the OpenSet and we found a better path
        neighborPtr->g_score = tentative_g_score;
        neighborPtr->f_score = tentative_g_score + getHeu(neighborPtr, endPtr);
        neighborPtr->Father = currentPtr;
        // Update the OpenSet
...
vector<Vector3d> Astarpath::getPath() {
  ...
  MappingNodePtr terminatePtr = Map_Node[goalIdx(0)][goalIdx(1)][goalIdx(2)];
