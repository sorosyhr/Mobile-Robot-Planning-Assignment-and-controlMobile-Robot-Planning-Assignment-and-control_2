                      ...
                      Quadrotor::InternalState& dxdt, const double /* t */)
{
  ...
  // v_dot = //?????
  // 计算线性加速度 v_dot
  Eigen::Vector3d gravity_effect = -Eigen::Vector3d(0, 0, g_);                      // 重力
  Eigen::Vector3d thrust_effect = thrust * R.col(2) / mass_;                        // 推力
  Eigen::Vector3d external_force_effect = external_force_ / mass_;                  // 外力
  Eigen::Vector3d drag_effect = -resistance * vnorm / mass_;                        // 空气阻力
  v_dot = gravity_effect + thrust_effect + external_force_effect + drag_effect;
  ...
  // omega_dot = //??????
// 计算角加速度 omega_dot
  Eigen::Vector3d coriolis_effect = cur_state.omega.cross(J_ * cur_state.omega);    // 科里奥利力
  Eigen::Vector3d torque_effect = moments + external_moment_;                       // 总力矩
  omega_dot = J_.inverse() * (torque_effect - coriolis_effect);
